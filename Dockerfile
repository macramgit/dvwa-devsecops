...
RUN apt-get update && \
    apt-get upgrade -y && \                # DODANE: aktualizacja systemu (m.in. linux-libc-dev)
    export DEBIAN_FRONTEND=noninteractive && \
    apt-get install -y zlib1g-dev libpng-dev libjpeg-dev libfreetype6-dev iputils-ping git && \
    apt-get clean && rm -rf /var/lib/apt/lists/*                            #  DODANE: czyszczenie cache apta
...
RUN cd /var/www/html/vulnerabilities/api && composer install || true     #  DODANE: || true zabezpiecza, gdy composer.json nie istnieje
